...
class Plugin {
  constructor() {
    _defineProperty(this, "config", {
      seamlessJumpToTitle: {
        ...
        type: 'boolean',
        description: 'Focus jumps from the editor to the note title bar by `vim:move-up` command',
        default: false
      }
    });
    ...
    _defineProperty(this, "handleVimModeChange", (event, _opt) => {
      _inkdrop.logger.debug('vim mode changed:', event);
      const {
        mode
      } = event;
    ...
    _defineProperty(this, "handleEditorTextInput", event => {
      // only process if the event is fired via EventTarget.dispatchEvent()
      if (this.isInsertMode() && !event.isTrusted) {
        _inkdrop.logger.debug('handle text input:', event);
        const text = event.data;
        const cm = this.getCodeMirror();
        cm.replaceSelection(text);
      }
    });
    _defineProperty(this, "handleEditorKeyDown", event => {
      const keyName = this.normalizeKeyName(event.key);
      const cm = this.getCodeMirror();
      const vim = this.vim.maybeInitVimState(cm);
      const isNumeric = !event.ctrlKey && !event.altKey && !event.metaKey && !event.shiftKey && keyName.match(/^\d$/);
      const target = cm.getInputField();
      const currentKeyStroke = [...inkdrop.keymaps.queuedKeystrokes.filter(k => !k.startsWith('^')), keyName];
      const {
        partialMatchCandidates,
        exactMatchCandidates
      } = inkdrop.keymaps.findMatchCandidates(currentKeyStroke);
      const partialMatches = inkdrop.keymaps.findPartialMatches(partialMatchCandidates, target);
      const exactMatches = inkdrop.keymaps.findExactMatches(exactMatchCandidates, target);
      _inkdrop.logger.debug('handleEditorKeyDown: currentKeyStroke:', currentKeyStroke);
      _inkdrop.logger.debug('handleEditorKeyDown: exactMatches:', exactMatches);
      _inkdrop.logger.debug('handleEditorKeyDown: partialMatches:', partialMatches);
      if (this.isBufferingKey()) {
        _inkdrop.logger.debug('handleEditorKeyDown: handle key buffering:', keyName, event);
        const b = cm.getInputField().webkitMatchesSelector('.CodeMirror.vim-mode:not(.insert-mode) textarea');
        _inkdrop.logger.debug('handleEditorKeyDown: keybinding check:', exactMatches, b);
        if (keyName !== 'Ctrl' && keyName !== 'Alt' && keyName !== 'Shift' && keyName !== 'Meta') {
          const {
            inputState
          } = vim;
          const hasOperatorOrMotion = inputState.motion || inputState.operator;
          if ((event.key.length === 1 || keyName === 'space') && (!isNumeric || !hasOperatorOrMotion)) {
    ...
    _defineProperty(this, "handleFocusEditor", _event => {
      this.yankClipboard();
    });
  }
  activate() {
    this.vim = (0, _keymap.default)(_codemirror.default);
  ...
  startBufferingKey(command, customBufferingModeClass) {
    ...
    _inkdrop.logger.debug('Start key buffering');
  ...
  stopBufferingKey() {
    _inkdrop.logger.debug('Stop key buffering');
    const wrapper = this.getCodeMirror().getWrapperElement();
  ...
  bufferKey(key) {
    ...
    _inkdrop.logger.debug('keyBuffer:', vim.inputState.keyBuffer, vim.inputState);
  }
  isInsertMode() {
    const wrapper = this.getCodeMirror().getWrapperElement();
  ...
  registerCommands() {
    ...
    const doKeyToKey = async command => {
      _inkdrop.logger.debug('doKeyToKey:', command);
      ...
      while (keys) {
        ...
        let key = match[0];
        _inkdrop.logger.debug('key:', key);
        keys = keys.substring(match.index + key.length);
        if (this.isBufferingKey()) {
          this.handleEditorKeyDown(new KeyboardEvent('keydown', {
            key
          }));
        } else {
          if (key.match(/^[A-Z]$/)) {
          ...
          const {
            exactMatchCandidates
          } = inkdrop.keymaps.findMatchCandidates([key]);
          _inkdrop.logger.debug('exactMatchCandidates:', exactMatchCandidates);
          const bindings = inkdrop.keymaps.findExactMatches(exactMatchCandidates, document.activeElement);
          const b = bindings[0];
          if (b) {
            inkdrop.commands.dispatch(document.activeElement, b.command);
          } else {
            _inkdrop.logger.debug('command not found for key:', key);
          }
        }
      }
    };
    ...
    const h = command => {
      return e => {
        _inkdrop.logger.debug('command:', command, 'state:', Object.assign({}, cm.state.vim.inputState));
        e.stopPropagation();
        const vim = this.vim.maybeInitVimState(cm);
        return cm.operation(() => {
          ...
          try {
            ...
            if (!_codemirror.default.Vim.suppressErrorLogging) {
              console.error(e);
            }
            throw e;
          }
    ...
    const b = command => {
      return e => {
        _inkdrop.logger.debug('buffer command:', command, 'state:', Object.assign({}, cm.state.vim.inputState), e.originalEvent);
    ...
    const p = command => {
      return e => {
        _inkdrop.logger.debug('operator command:', command, 'state:', Object.assign({}, cm.state.vim.inputState), e.originalEvent, e);
        const vim = this.vim.maybeInitVimState(cm);
        if (!vim.inputState.operator && !this.isBufferingKey() && !vim.visualMode) {
          ...
          if (e.originalEvent) {
            ...
            _inkdrop.logger.debug('buffer key:', command.keys);
    ...
    const handlers = {
      ...
      'vim:reset-normal-mode': e => {
        _codemirror.default.Vim.clearInputState(cm);
        this.stopBufferingKey();
        e.stopPropagation();
      },
      'vim:exit-visual-mode': e => {
        _inkdrop.logger.debug('exit-visual-mode');
        _codemirror.default.Vim.clearInputState(cm);
        _codemirror.default.Vim.exitVisualMode(cm);
        this.stopBufferingKey();
        e.stopPropagation();
      },
      'vim:exit-insert-mode': e => {
        _inkdrop.logger.debug('exit-insert-mode');
        _codemirror.default.Vim.clearInputState(cm);
        _codemirror.default.Vim.exitInsertMode(cm);
        this.stopBufferingKey();
        e.stopPropagation();
      },
      'vim:move-left': h({
    ...
    disposables.add(inkdrop.commands.add(document.querySelector('.mde-preview'), {
      'vim:move-to-start-of-file': ({
        target
      }) => {
        target.scrollTop = 0;
      },
      ...
      'vim:scroll-half-screen-up': ({
        target
      }) => {
        target.scrollTop -= target.clientHeight / 2;
      },
      'vim:scroll-half-screen-down': ({
        target
      }) => {
        target.scrollTop += target.clientHeight / 2;
      },
    ...
    disposables.add(new _eventKit.Disposable(() => wrapper.removeEventListener('keydown', this.handleEditorKeyDown)));
