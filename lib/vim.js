...
let Plugin = (_class = class Plugin {
  ...
  registerCommands() {
    ...
    const h = command => {
      return e => {
        debug('command:', command, 'state:', Object.assign({}, cm.state.vim.inputState));
        e.stopPropagation();
        const vim = this.vim.maybeInitVimState(cm);
        return cm.operation(() => {
    ...
    const b = command => {
      return e => {
        debug('buffer command:', command, 'state:', Object.assign({}, cm.state.vim.inputState), e.originalEvent);
    ...
    const p = command => {
      return e => {
        debug('operator command:', command, 'state:', Object.assign({}, cm.state.vim.inputState), e.originalEvent);
        const vim = this.vim.maybeInitVimState(cm);
    ...
    disposables.add(inkdrop.commands.add(editor.textareaRef.nextSibling, handlers));
